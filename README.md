...
- **The collector** (`bzm-vitals.ts`) — rides along with your test and records
...
Put `bzm-vitals.ts` next to your spec file, then change one line in the spec:
...
`package.json` — **plus `bzm-vitals.ts`**. It's just one more file in the
upload. Run the test.
...
*Today the collector is distributed as a file you copy (`bzm-vitals.ts`,
