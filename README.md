...
*The collector is on npm as
[`bzm-playwright-vitals`](https://www.npmjs.com/package/bzm-playwright-vitals)
(with [`bzm-vitals-format`](https://www.npmjs.com/package/bzm-vitals-format)
as its one dependency). For environments with no registry access there is
also a single-file standalone build — see
[`packages/collector/upload-kit`](packages/collector/upload-kit). A runnable
`bzm-vitals-dashboard` CLI package is still to come; today the dashboard runs
from this repo as shown above.*
